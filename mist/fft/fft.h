...
void fft( array1< T1, Allocator1 > &in, array1< std::complex< T2 >, Allocator2 > &out )
{
		...
		void fft( array1< std::complex< T1 >, Allocator1 > &in, array1< std::complex< T2 >, Allocator2 > &out )
	{
		...
		double *w;
		int *ip;
		type_limits< T2 > over_flow_check;
		...
		for( i = 0 ; i < out.size() ; i++ )
		{
			if( over_flow_check.maximamu() < (T2)data[2*i] ||
				over_flow_check.maximamu() < (T2)data[2*i+1] ||
				over_flow_check.minimamu() > (T2)data[2*i] ||
				over_flow_check.maximamu() < (T2)data[2*i+1] )
			{
				cout << "over flow in fft\n";
			}
			out[i] = std::complex< T2 >( (T2)data[2*i], (T2)data[2*i+1] );
		}
...
void inverse_fft( array3< T1, Allocator1 > &in, array3< std::complex< T2 >, Allocator2 > &out )
{
	...
	for( i = 0 ; i < in.width() ; i++ )
	{
		for( j = 0 ; j< in.height() ; j++ )
		{
			for( k = 0 ; k < in.depth()*2 ; k++ )
			{
				data[i][j][k] *= 1.0/( in.width() * in.height() * in.depth() );
			}
		}
	}
